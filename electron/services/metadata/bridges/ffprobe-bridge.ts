...
export class FfprobeBridge {
  ...
  constructor(ffprobePath: string) {
    ...
    log.info(`[FfprobeBridge] Initialized with binary: ${ffprobePath}`);
  }
  ...
  probe(filePath: string): Promise<ProbeResult> {
    return new Promise((resolve, reject) => {
      ...
      log.info(`[FfprobeBridge] Probing: ${filePath}`);
      ...
      proc.on('close', (code) => {
        if (code !== 0) {
          log.error(`[FfprobeBridge] Failed with code ${code}: ${stderr}`);
        ...
        try {
          ...
          log.info(`[FfprobeBridge] Probe complete: ${result.streams?.length || 0} streams`);
          resolve(result);
        } catch (e) {
          log.error(`[FfprobeBridge] Failed to parse output: ${e}`);
          reject(new Error(`Failed to parse ffprobe output: ${e}`));
        }
      });

      proc.on('error', (err) => {
        log.error(`[FfprobeBridge] Spawn error: ${err.message}`);
