...
class DCCamGenerator:
    ...
    def generate_dc_cam_compound(self, compound_xml_path: str, audio_sources: Dict[str, str],
        ...
        for audio_type, audio_path in audio_sources.items():
            if audio_path and audio_type in ['mic1', 'mic2', 'mic3', 'mic4', 'soundEffects']:
                try:
        ...
        if enable_master_audio:
            print("  Master-only mode: enabling master audio in DC CAM compound")
        ...
        if cam1_config:
            ...
            if cam1_asset_id:
                ...
                print("  Stream recovery mode: using stream layout transforms for DC CAM camera 1")
                cam1_transforms = {
                    'crop': [3.31595, 60.72, 108.823, 2.63889],
                    'crop_mode': 'trim',
                    'transform': {
                        'position': [49.0484, 64.7271],
                        'scale': 1.58709
                    }
                }
            else:
        ...
        if cam2_config:
            ...
            if cam2_asset_id:
                ...
                print("  Stream recovery mode: using stream layout transforms for DC CAM camera 2")
                cam2_transforms = {
                    'crop': [104.484, 2.08216, 9.76545, 62.2644],
                    'crop_mode': 'trim',
                    'transform': {
                        'position': [-43.2076, -68.0996],
                        'scale': 1.64043
                    }
                }
            else:
