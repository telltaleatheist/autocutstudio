...
class ShortsMasterProjectGenerator:
    """Generate master projects combining CAM and SSB compound clips (no GS for vertical shorts)."""
    ...
    def generate_shorts_master_project(self, cam_xml_path: str,
                                   ssb_xml_path: str, original_name: str,
                                   output_path: Optional[str] = None) -> List[str]:
        """Generate SHORTS master project combining CAM and SSB compounds (no GS for vertical).

        Returns:
            List of paths to generated project files (may be multiple if split into parts)
        """
        return self._generate_shorts_master_project(
            cam_xml_path, ssb_xml_path,
            original_name, "SHORTS", output_path
        )

    def generate_dc_shorts_master_project(self, cam_xml_path: str,
                                 ssb_xml_path: str, original_name: str,
                                 output_path: Optional[str] = None) -> List[str]:
        """Generate DC (Dual Camera) master project combining CAM and SSB compounds (no GS for vertical).
        ...
        return self._generate_shorts_master_project(
            cam_xml_path, ssb_xml_path,
            original_name, "DC_SHORTS", output_path
        )
    ...
    def _generate_shorts_master_project(self, cam_xml_path: str,
                                ssb_xml_path: str, original_name: str,
        ...
        Note: Shorts only include CAM and SSB (no GS for vertical format).
        ...
        # Extract all resources from both XMLs
        all_resources = {}
        for xml_path in [cam_xml_path, ssb_xml_path]:
        ...
        print(f"Shorts master project saved: {output_file}", file=sys.stderr)
    ...
    def _generate_combined_project(self, cam_media: ET.Element,
        ...
        Args:
            ...
            shared_uids: Dict with 'cam', 'ssb', 'event' UIDs
        ...
        # Note: No GS compound for shorts - only CAM and SSB

        # Add other resources
        skip_ids = {'r1', 'r2', 'r3', 'r6'}
        for resource_id, resource in all_resources.items():
            if resource_id not in skip_ids and resource.tag != 'media':
                resources_elem.append(resource)
